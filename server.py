...
Request body::

    {
        "outlet": "hindustan_times" | "business_standard" | "news18" |
                  "firstpost" | "republic_world",            // required
...
*Why no "all‑outlets" option?*  Upstream orchestration distributes calls across
multiple instances (or across time) instead of burdening a single FastAPI
process.
"""
...
app = FastAPI(title="Shottify Scraper API", version="0.1.0", docs_url="/")
...
class ScrapeRequest(BaseModel):
    ...
    page_size: int = Field(
        ...
        ge=1,
        le=100,
        description="Results requested per remote page (1–100)",
    )
...
async def _run_scraper(
    ...
    """Execute *scraper_cls.search* in executor and consolidate pages."""
    ...
    def _blocking_call() -> List[Article]:
        ...
        while len(collected) < limit:
            size = min(page_size, limit - len(collected))
            batch = scraper.search(keyword, page=page, size=size)
            if not batch:
                break
